# GPU 功率固定分配守护程序 (gpu-power-allocator)
...
| | gpu-idle-saver | gpu-power-allocator |
|---|---|---|
...
> 当前生产机已启用 gpu-power-allocator 并禁用 gpu-idle-saver。

## 安装部署

```bash
# 1. 同步到部署目录
sudo mkdir -p /opt/gpu-power-allocator
sudo cp gpu_power_allocator.py config.ini /opt/gpu-power-allocator/
sudo python3 -m py_compile /opt/gpu-power-allocator/gpu_power_allocator.py

# 2. 安装 systemd 服务（unit 内 ExecStart 路径指向 /opt/gpu-power-allocator）
sudo cp gpu-power-allocator.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now gpu-power-allocator

# 3. 先禁用旧服务（若还在运行）
sudo systemctl disable --now gpu-idle-saver

# 4. 查看运行状态与日志
systemctl status gpu-power-allocator
journalctl -u gpu-power-allocator -f
```
...
修改配置后：`sudo systemctl restart gpu-power-allocator`
...
journalctl -u gpu-power-allocator -n 5   # 可见 "自愈纠偏下发 [GPU3:185W]"
```

## 安全机制

- **单实例锁**：`/var/run/gpu-power-allocator.lock`，防止重复进程互相覆盖
