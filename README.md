...
> 当前版本 **v0.3.2**（tag: `v0.3.2`）· 忙卡均分剩余预算（PP 流水线适配）+ 下发死区。
> 各版本变更见底部 [版本记录](#版本记录)。
...
- 忙卡**均分**"预算 − 空闲占用"——适配 PP(流水线)推理：每卡同质负载、串联执行，
  **瓶颈卡决定全链路吞吐**，谁吃得少谁就是短板，故每张忙卡分到相同的功耗上限，
  谁都不被饿着；物理上限不足的卡 clamp 到其 `max`，剩余预算再分给能接收的卡；
- 数学上保证总和恒 ≤ `power_budget`，无论几张卡、卡多卡少、`power_low` 设多大。

> 注：旧版本曾按 `power.draw` 加权"谁用得多谁拿得多"，属正反馈——同质负载下会
> 喂饱的越饱、饿的越饿，恰好拖慢 PP 流水线；v0.3.2 改为均分。
...
每个采样周期：
- **空闲卡** → 固定 `power_low`；
- **忙卡** → **均分** "`power_budget` − 空闲卡占用"，每张忙卡上限一致；
  某卡物理上限（`max`）低于份额时 clamp 到 `max`，超出部分再分给其他忙卡。
...
*文档对应版本：v0.3.2 · 整机 GPU 硬预算默认 750W（电源1200W 预留450W给CPU等），忙卡均分 + 下发死区*